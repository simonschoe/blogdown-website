...
And indeed, a quick glance at the missing values per feature reveals numerous missing values for the `price`  asfasfsa `price` (`r (gdpr_data %>% map_df(., ~sum(is.na(.))) %>% pull(price) / dim(gdpr_data)[[1]]) %>% scales::percent(accuracy = 0.01)`), `date` (`r (gdpr_data %>% map_df(., ~sum(is.na(.))) %>% pull(date) / dim(gdpr_data)[[1]]) %>% scales::percent(accuracy = 0.01)`) and `entity` (`r (gdpr_data %>% map_df(., ~sum(is.na(.))) %>% pull(entity) / dim(gdpr_data)[[1]]) %>% scales::percent(accuracy = 0.01)`) feature. Without diving deeper into the information sources, it may be assumed that for the affected court cases no complete record of the verdict was openly published by the jurisdiction.
...
Having identified the first ever fine the naturally arising  
Top 10 fines
```{r top10}
gdpr_data %>%
  select(-summary, -picture) %>% 
...
[^1]: SAP.2020
[^2]: EuropeanUnion.2018
[^3]: PrivacyAffairs.2020
[^4]: @Wickham.2020
[^5]: @vanderLoo.2014
